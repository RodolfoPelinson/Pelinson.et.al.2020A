...
community <- read.csv("Not commit/community_data.csv", header = T)
treatments <- read.csv("Not commit/treatments_coordnates.csv", stringsAsFactors = T, row.names = 1)
traits <- read.csv("Not commit/species_data.csv", header = T, stringsAsFactors = T)


#Constucting matrices------------------------------------------------------------------------------------------------
#Fish and Isolation variables####
coord <- treatments[,3:4]
fish <- as.factor(rep(treatments$fish,3))
isolation <- rep(treatments$isolation,3)
isolation <- factor(isolation, levels = c("30","120","480"))
SS <- na.omit(as.factor(community$SS))
ID <- as.factor(community[3:dim(community)[1],1])


#Constructing Y matrix####
com <- community[,c(3:length(community))]
species_inf <- t(com[c(1:2),])
com <- com[c(3:dim(community)[1]),]
...
#These are just unecessary lines in the matrix where I joinned all choronominae and tanypodinae. We ended up separeting them.
